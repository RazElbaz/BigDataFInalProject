...
createDaysCitySales(new Date(2022, 4, 26));
const stream = fs.createWriteStream("local_sales_history.csv");
...
async function createDaysCitySales(target_date){
    ...
    while (today.getTime() !== target_date.getTime()) {
        ...
        for (city of cities) {
            ...
            await createCitySales(city, target_date, probabilities, salesArr);
        }
...
async function createCitySales(cityName, date, salesArr) {
    ...
    while(currentTotalAmount < endRange){
        ...
        let saleJSON = {
            ...
            'holiday_weak': holiday_week,
            'date': `${day}-${month}-${year}`,
            'season': season,
            'weather': randWeather
        }
        let fields = Object.keys(saleJSON)
        await write('local_sales_history.csv', fields, saleJSON);
...
async function write (fileName, fields, data) {
